...
def predict_digit(model, image_tensor):
    with torch.no_grad():
        ...
        return pred.item(), prob.item(), probs.squeeze().numpy(),time.time() - start_time

# -------------------- 5. 可视化结果 --------------------
def visualize_prediction(image_tensor, prediction, confidence, all_probs, output_path):
...
if __name__ == "__main__":
    ...
    for img_path in image_paths:
        ...
        predicted_digit, confidence, all_probs,inference_time = predict_digit(model, input_tensor)
        ...
        visualize_prediction(input_tensor, predicted_digit, confidence, all_probs, output_path)
