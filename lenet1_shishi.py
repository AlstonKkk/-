...
# 新增图像编码函数
def encode_image_to_bytes(figure):
    """将matplotlib图像转为字节流"""
    from io import BytesIO
    buf = BytesIO()
    figure.savefig(buf, format='png', bbox_inches='tight', dpi=72)
    buf.seek(0)
    return buf.getvalue()
# -------------------- 5. 可视化结果 --------------------
def visualize_prediction(image_tensor, prediction, confidence):
    ...
    plt.show()
    # 转换为字节流
    img_bytes = encode_image_to_bytes(plt.gcf())
    plt.close()

    # 特殊格式输出（带长度前缀）
    sys.stdout.buffer.write(b"IMG_START")
    sys.stdout.buffer.write(len(img_bytes).to_bytes(4, "big"))  # 4字节长度
    sys.stdout.buffer.write(img_bytes)
    sys.stdout.buffer.write(b"IMG_END")
    sys.stdout.flush()
